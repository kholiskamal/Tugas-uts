...
btnPlayerImg.forEach((m) => {
  m.addEventListener("click", function () {
    ...
    setTimeout(
      () =>
        (compArea.innerHTML = `<img src="${compImg}.png" alt="batu" class="img-player-js">`),
      700
    );
...
function hidePopupBox() {
  ...
  document.querySelector(".popup-container").style.animation =
    "hide-popup-container .5s ease 1 forwards";
  document.querySelector(".popup-box").style.animation =
    "hide-popup-box .5s ease 1 forwards";
...
function gameResults(playerImg, compImg) {
  ...
  if (playerImg == `kertas`) {
    if (compImg == `gunting`) {
      resultText = "kamu kalah dan komputer menang,SEMANGAT";
      scorTxt = "KOMPUTER MENDAPATKAN 1 SKOR 😔";
      scorComp += 1;
    }
    if (compImg == `batu`) {
  ...
  setTimeout(() => {
    ...
    popupContainer.innerHTML = popupBoxCodes(
      playerImg,
      compImg,
      resultText,
      scorTxt
